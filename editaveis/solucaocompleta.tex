...
Nesta seção será descrito o desenvolvimento da solução e as técnicas utilizadas. A figura \ref{fig:dfd} mostra o diagrama de fluxo de dados com todas as técnicas e procedimentos implementados no sistema-solução. 

Como é mostrado na figura \ref{fig:dfd}, o fluxo de controle do sistema-solução começa com a chamada da função \textbf{DA3}. Ela recebe como parâmetro um vetor de audio $stereo$, ou seja, ela carrega uma matriz 2 por $N$, tal que $N$ é o tamanho do sinal de áudio (número de amostras). Esse sinal de áudio é retornado através da função $wavread$($<$$caminho$\_$do$\_$arquivo$$>$$)$. O tipo de arquivo lido é do formato-padrão de áudio .wav. Esse formato de arquivo permite um armazenamento dos dados em blocos em modulação de pulsos PCM ($Pulse$ $Code$ $Modulation$). O PCM armazena em arquivo de áudio não-comprimido (sem perdas), ou seja, o processo de amostragem e quantização representa exatamente o que foi descrito na parte de fundamentos teóricos desse trabalho (taxa de amostragem de 44.100 Hz e quantização de 16 bits). Ao final do fluxo há 2 informações extraídas nas saídas: os acordes ao longo do tempo e a tonalidade da música. Nos próximos tópicos serão explicados os detalhes de cada um dos procedimentos e técnicas do diagrama de fluxo de dados.
...
\subsection{Ciclo 2: Realocação das Frequências em 1 Unidade de Hz}
...
\subsection{Ciclo 3: Frequências e Notas Musicais}
...
\subsection{Ciclo 4: Determinação dos Acordes Fundamentais}
...
\subsection{Ciclo 5: Detecção de Transições Rítmicas}
...
\subsection{Ciclo 6: Aplicação da Transformada Wavelets}
...
\subsection{Ciclo 7: Aplicação da Transformada de Fourier Janelada}
...
\subsection{Ciclo 8: Extração de Tonalidade}
...
\subsection{Ciclo 9: Binarização de Energia das Notas}
...
\subsection{Ciclo 10: Segmentação de Áudio}
...
\subsection{Ciclo 11: Extração de Baixos}
...
\subsection{Ciclo 12: Extração de Acordes Invertidos e Aumentados}
